...
- `ulanziRestart`: launches a PowerShell helper that resolves the Ulanzi Studio install path, stops only Ulanzi-owned processes under that install root, and starts `UlanziDeck.exe` again. The helper stops the main app before child processes and skips its own process ancestry so a button-triggered restart cannot kill the plugin host before the helper finishes. The action listens to both `keydown` and `run` with a short duplicate guard, and writes restart diagnostics to `%LOCALAPPDATA%\UlanziUtilitySuite\ulanzi-restart\restart.log`.
